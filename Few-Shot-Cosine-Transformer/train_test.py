...
if __name__ == '__main__':
    ...
    elif params.dataset == 'Sea':  # Add Sea dataset
        sea_data_dir = os.path.join(os.getcwd(), 'dataset', 'Sea')
        base_file = os.path.join(sea_data_dir, 'base.json')
        val_file = os.path.join(sea_data_dir, 'val.json')
        test_file = os.path.join(sea_data_dir, 'novel.json')

        # Ensure JSON files exist
        if not (os.path.exists(base_file) and os.path.exists(val_file) and os.path.exists(test_file)):
            raise FileNotFoundError("Sea dataset JSON files not found. Ensure `base.json`, `val.json`, and `novel.json` exist in the dataset directory.")
    else:
    ...
    if params.dataset == 'cross':
        if params.split == 'base':
            testfile = configs.data_dir['miniImagenet'] + 'all.json'
        else:
            testfile = configs.data_dir['CUB'] + params.split + '.json'
    elif params.dataset == 'cross_char':
        if params.split == 'base':
            testfile = configs.data_dir['Omniglot'] + 'noLatin.json'
        else:
            testfile = configs.data_dir['emnist'] + params.split + '.json'
    elif params.dataset == 'Sea':  # Test Sea dataset
        testfile = test_file
    else:
        testfile = configs.data_dir[params.dataset] + params.split + '.json'
